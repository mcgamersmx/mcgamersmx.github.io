(function($) {
  var settings = {carousels:{speed:4, fadeIn:true, fadeDelay:250}};
  ...
  $(function() {
    ...
    $("#nav > ul").dropotron({mode:"fade", speed:350, noOpenerFade:true, alignment:"center"});
    $(".scrolly").scrolly();
    $('<div id="navButton">' + '<a href="#navPanel" class="toggle"></a>' + "</div>").appendTo($body);
    $('<div id="navPanel">' + "<nav>" + $("#nav").navList() + "</nav>" + "</div>").appendTo($body).panel({delay:500, hideOnClick:true, hideOnSwipe:true, resetScroll:true, resetForms:true, target:$body, visibleClass:"navPanel-visible"});
    if (skel.vars.os == "wp" && skel.vars.osVersion < 10) {
      $("#navButton, #navPanel, #page-wrapper").css("transition", "none");
    }
    $(".carousel").each(function() {
      var $t = $(this), $forward = $('<span class="forward"></span>'), $backward = $('<span class="backward"></span>'), $reel = $t.children(".reel"), $items = $reel.children("article");
